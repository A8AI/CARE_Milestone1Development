...
export default class Care_MultiPayPeriodsTwoForm extends LightningElement {
    @api idCareApp;
    ...
    wiredPickListValue({
        ...
        if (data) {
            ...
            PeriodData = PeriodData.filter(function (obj) {
                return obj.value !== 'Year to Date' && obj.value !== 'Not Applicable';
            });
    ...
    updateAnnualSalary() {
        ...
        if (this.objInputFields.sPeriodType === "Annually") {
            ...
            let finalaverageAmount = Math.round(averageAmount);
            this.objInputFields.dTotalAmount = finalaverageAmount;
        } else if (this.objInputFields.sPeriodType === "Bi Weekly") {
            let averageAmount = (this.objInputFields.dAmount1 + this.objInputFields.dAmount2) / 2;
            let finalAmount = averageAmount * 26;
            ...
            let averageAmount = (this.objInputFields.dAmount1 + this.objInputFields.dAmount2) / 2;
            let finalAmount = averageAmount * 24;
            let finalAmountRoundedOf = Math.round(finalAmount);
            this.objInputFields.dTotalAmount = finalAmountRoundedOf;
        } else if (this.objInputFields.sPeriodType === "Weekly") {
            let averageAmount = (this.objInputFields.dAmount1 + this.objInputFields.dAmount2) / 2;
            let finalAmount = averageAmount * 52;
            let finalAmountRoundedOf = Math.round(finalAmount);
            this.objInputFields.dTotalAmount = finalAmountRoundedOf;
        }
    }
